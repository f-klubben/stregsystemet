...
def razzia(request, razzia_id, razzia_type=BreadRazzia.BREAD, title=None):
    if request.method == "POST":
        return razzia_view_single(
            request,
            razzia_id,
            request.POST["username"],
            razzia_type=razzia_type,
            title=title,
        )
    else:
        return razzia_view_single(
            request, razzia_id, None, razzia_type=razzia_type, title=title
        )


def _sales_to_user_in_period(
    username, start_date, end_date, product_list, product_dict
):
    result = (
...
@permission_required("stregreport.host_razzia")
def razzia_view_single(
    request, razzia_id, queryname, razzia_type=BreadRazzia.BREAD, title=None
):
    ...
    if razzia_type == BreadRazzia.FNUGFALD:
        ...
        sales_to_user = _sales_to_user_in_period(
            queryname, start_date, end_date, product_list, product_dict
        )
    ...
    entries = list(
        razzia.razziaentryold_set.filter(member__pk=member.pk).order_by("-time")
    )
    ...
    if not already_checked_in or (
        (razzia_type == BreadRazzia.FOOBAR or razzia_type == BreadRazzia.FNUGFALD)
        and not within_wait
    ):
...
def new_razzia(request, razzia_type=BreadRazzia.BREAD):
    ...
    views = {
        ...
        BreadRazzia.FNUGFALD: "fnugfald_view",
    }
...
def razzia_wizard(request):
    ...
    return render(
        ...
        {
            "start_date_picker": start_date_picker.widget.render(
    ...
    for cat in Category.objects.all():
        stat_lists.append(
            (
                cat.name,
                sale_product_rank(
                    get_product_ids_from_category(cat), from_time, to_time
                ),
            )
        )
...
def sale_money_rank(from_time, to_time, rank_limit=10):
    stat_list = (
        Member.objects.filter(
            active=True, sale__timestamp__gt=from_time, sale__timestamp__lte=to_time
        )
...
def fjule_party(year):
    first_december = timezone.datetime(
        year, 12, 1, 22, tzinfo=pytz.timezone("Europe/Copenhagen")
    )
...
def daily(request):
    ...
    latest_sales = Sale.objects.prefetch_related("product", "member").order_by(
        "-timestamp"
    )[:7]
    top_today = (
        Product.objects.filter(sale__timestamp__gt=current_date)
        .annotate(Count("sale"))
    ...
    revenue_day = (
        Sale.objects.filter(timestamp__gt=startTime_day).aggregate(Sum("price"))[
            "price__sum"
        ]
    ) or 0.0
    startTime_month = timezone.now() - datetime.timedelta(days=30)
    revenue_month = (
        Sale.objects.filter(timestamp__gt=startTime_month).aggregate(Sum("price"))[
            "price__sum"
        ]
    ) or 0.0
    top_month_category = (
