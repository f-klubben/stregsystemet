...
def replace_4xx_parameter_values(transaction):
    ...
    if transaction["expected"]["statusCode"][0] == "4":
        new_path = update_query_parameter_values(
            transaction["fullPath"], not_found_parameter_values
        )
...
def replace_body_in_post_requests(transaction):
    if transaction["expected"]["statusCode"][0] == "4" and transaction["id"].startswith(
        "POST"
    ):
