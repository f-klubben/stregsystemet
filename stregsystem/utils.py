...
def make_active_productlist_query(queryset):
    ...
    candidates_out_of_stock = (active_candidates
...
def make_inactive_productlist_query(queryset):
    ...
    inactive_candidates = (
        queryset.exclude(Q(active=True) & (Q(deactivate_date=None) | Q(deactivate_date__gte=now))).values("id"))
    inactive_out_of_stock = (queryset
                             .filter(sale__timestamp__gt=F("start_date"))
                             .annotate(c=Count("sale__id"))
                             .filter(c__gte=F("quantity"))
                             .values("id")
