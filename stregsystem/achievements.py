...
def get_new_achievements(
    member: Member, product: Product, amount: int = 1
) -> List[Achievement]:
    ...
    completed_achievement_ids = completed_achievements.values_list(
        "achievement_id", flat=True
    )
    in_progress_achievements = Achievement.objects.exclude(
        id__in=completed_achievement_ids
    )
    ...
    completed_achievements: List[Achievement] = _find_completed_achievements(
        related_achievement_tasks, member, now
    )
...
def _find_completed_achievements(
    ...
    task_to_sales: Dict[int, QuerySet[Sale]] = _filter_sales(
        related_achievement_tasks, member, now
    )
        ...
        for at in group:
            ...
            if task_type == "default" or task_type == "any":
                ...
                if at.alcohol_content and alcohol_promille < (at.goal_count / 100):
                    ...
                    not at.alcohol_content and not at.caffeine_content
...
def _filter_sales(
    achievement_tasks: List[AchievementTask], member: Member, now: datetime
) -> Dict[int, QuerySet[int]]:

    # Prefetch product and categories to reduce DB hits later
    sales_qs = (
        Sale.objects.filter(member=member)
...
def _filter_achievement_tasks(
    ...
    achievements_with_constraints = in_progress_achievements.prefetch_related(
        "achievementconstraint_set"
    )

    # Step 1: Filter achievements that are currently "active"
    active_achievements = [
        a for a in achievements_with_constraints if _is_achievement_active(a, now)
    ]
