...
urlpatterns = [
    ...
    re_path(
        r"^(?P<room_id>\d+)/sale/(?P<member_id>\d+)/$", views.menu_sale, name="menu"
    ),
    re_path(
    ...
    re_path(
        r"^(?P<room_id>\d+)/user/(?P<member_id>\d+)/$",
        views.menu_userinfo,
        name="userinfo",
    ),
    re_path(
        r"^(?P<room_id>\d+)/user/(?P<member_id>\d+)/pay$",
        views.menu_userpay,
        name="userpay",
    ),
    re_path(
        r"^(?P<room_id>\d+)/user/(?P<member_id>\d+)/rank$",
        views.menu_userrank,
        name="userrank",
    ),
    re_path(
        r"^(?P<room_id>\d+)/send_csv_mail/(?P<member_id>\d+)/$",
        views.send_userdata,
        name="send_userdata",
    ),
    ...
    re_path(
        r"^api/products/named_products$",
        views.get_named_products,
        name="api_named_products",
    ),
    re_path(
        r"^api/products/active_products$",
        views.get_active_items,
        name="api_active_products",
    ),
    re_path(
        r"^api/products/category_mappings$",
        views.get_product_category_mappings,
        name="api_product_mappings",
    ),
